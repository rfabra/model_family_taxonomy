...
#         "ada", "caTools", "adabag", "ipred", "bst", "randomForest", "RRF", "pls", "KODAMA","caret", "h2o", "klaR")
...
mikappa<-function (a,b)
{
  res<-NA
if(sum(is.na(a))<1 && sum(is.na(b))<1)
{
  if ((sum(a==FALSE)+sum(b==FALSE))==0)
  {
    if (length (b)==length (a))
    {
      kk<-caret::confusionMatrix(b, a, mode="everything")
      #print(kk$overall["Kappa"])
      if (!is.nan(kk$overall["Kappa"])) 
      {
       res<-kk$overall["Kappa"]
      }
    }
  }
}
