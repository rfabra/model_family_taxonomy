...
datasets <- list.files('datasets/all', pattern=".arff")
nds = length(datasets)
...
for (ds in datasets) {
  ...
  if (csv_data[["selected?"]][[ids]] == "YES") {
    save_dataset(current_data, gsub(".arff", ".csv", sprintf("datasets/clean/%s", ds)))
  }
}
write.table(x=csv_data, file="files/analytics/datasets_clean.csv", row.names = FALSE, col.names = TRUE, sep=",")